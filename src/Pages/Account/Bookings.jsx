...
export default function Bookings() {
  const upcomingBookings= useSelector(state => state.persistedReducer.bookings)[0];
  const pastBookings = useSelector(state => state.persistedReducer.bookings)[1];
  const token = useSelector(state => state.persistedReducer.auth.token)
  const bookingType = useParams()
  const bookings = bookingType.confirmed==='confirmed' ? upcomingBookings : pastBookings;
    ...
    return(
      <tr key={i} className='booking-row'>
        ...
        <td>
            ...
            {bookingType.confirmed === 'confirmed' && booking.status !== 'cancelled'
              ? <div onClick={() => cancelBooking(booking.id, booking.venueData.location)} className='booking-detail pointer'>Cancel Booking</div>
              : <></>
            }
        </td>
      </tr>
